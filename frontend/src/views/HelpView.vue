            ...
            <table class="help-table">
              <tbody>
                ...
                <tr>
                  <td>加入群组 / 订阅频道</td>
                  <td>
                    加入群组或订阅频道，支持公开用户名（<code>@name</code>）或私有邀请链接。订阅频道可先校验当前订阅状态（已订阅则直接成功），发送请求后再次验证；加入群组可选配"入群后点击验证按钮"以完成部分群组的入群验证，验证消息出题（如「9 - 1 = ?」配一排数字按钮）时可勾选<strong>由 AI 判断该点哪个按钮</strong>（即 <code>{aiBtn}</code>），由 AI 读消息后挑按钮，拒绝/举报类按钮不会交给 AI。
                  </td>
                </tr>
                <tr>
                  <td>向联系人发送 / 点击按钮</td>
