...
const zh = {
  ...
  accounts: {
    ...
    filters: {
      ...
      anyStatus: "全部认证状态",
      needsAuth: "需要认证（非已认证）",
      anyRestriction: "全部限制状态",
      restrictedAny: "受限（任意）",
      unchecked: "未检测",
      anyEmail: "全部邮箱状态",
      emailBemby: "有 Bemby 邮箱",
      emailHas: "有登录邮箱",
      emailNone: "无登录邮箱",
      anyPasskey: "全部通行密钥状态",
      passkeyBemby: "有 Bemby 通行密钥",
      passkeyHas: "有通行密钥",
      passkeyNone: "无通行密钥",
      enabledLabel: "启用状态",
      anyEnabled: "启用与禁用",
      ...
      proxyAssigned: "已分配代理",
      proxyNone: "未分配代理",
      ownershipLabel: "来源与归属",
      anyOwnership: "全部来源",
      ownershipImported: "会话导入",
      ownershipPending: "待接管（导入且未接管）",
      ownershipTaken: "已接管",
      ownershipManual: "手动添加",
    },
    ...
    importCards: {
      btn: "卡密导入",
      title: "卡密导入（协议号）",
      hint: "每行一张卡密，格式为 手机号----接码链接（或直接粘贴链接/UUID）。系统会调用转换服务把卡密转成会话，验证登录后直接以「已认证」状态保存账户，无需接收验证码。设备指纹与 API ID 会按卡密自带的值锁定，请勿在官方客户端登录这些账户。",
      start: "开始导入",
      startFailed: "启动导入失败",
      notesLabel: "备注模板",
      notesHint: "{phone} 与 {uuid} 会替换为每个账户对应的值",
      aliveCheckLabel: "转换前先筛活",
      aliveCheckHint: "先检测卡密是否存活，避免把死号/冻结号转换掉——下载协议文件后将无法再自助补号。",
      proxyHint: "验证登录时使用的代理，随机分配；留空则走全局出口。仅支持 SOCKS。",
      converterUrlLabel: "转换服务地址",
      converterUrlPlaceholder: "https://your-converter.example",
      converterUrlHint: "卡密转会话所用的服务地址，由你自行填写；系统会记住上次填写的值，不会写入代码。",
      status: {
        pending: "等待中",
        checking: "筛活中",
        frozen: "已冻结",
    ...
    takeOwnership: {
      btn: "接管账户所有权",
      title: "接管账户所有权（协议号）",
      intro: "将对所选 {n} 个卡密导入的账户执行接管：用账户自带的原始二步验证密码改成你自己的、终止其他所有会话、移除非本系统的通行密钥。完成后卖家将无法再访问。",
      skippedHint: "已跳过 {n} 个非卡密导入的账户（仅协议号可接管）。",
      randomiseLabel: "为每个账户生成随机密码",
      randomiseHint: "每个账户使用独立强随机的二步验证密码（保存在系统内），比统一密码更安全。",
      newPassword: "新二步验证密码",
      repeatPassword: "重复新密码",
      removePasskeys: "移除非本系统的通行密钥",
      addPasskey: "接管后注册本系统的通行密钥",
      notesPlaceholder: "已接管",
      warning: "此操作会真实修改 Telegram 上的二步验证密码并踢下其他会话，不可撤销。建议账户导入 3-5 天后再执行。执行前切勿在官方客户端登录这些账户。",
      start: "开始接管",
      noTargets: "所选账户中没有卡密导入的已认证账户。",
    },
    bulkEmail: {
      btn: "批量修改登录邮箱",
      title: "批量修改登录邮箱",
  ...
  jobs: {
    ...
    runEveryDaysOneTimeHint: "一次性任务：从今天起算，在这几天内随机选一天执行（0 为今天，如 0-5）；不看上次成功时间，成功一次后停用",
    labelOneTime: "一次性任务",
    ...
    custom: {
      ...
      actionSaveMessageUrl: "保存消息中的链接为变量",
      ...
      labelVarName: "变量名",
      saveUrlVarHint:
        "取到的网址存为 {v}，之后的“打开网址”或网页步骤里直接写 {v} 即可。除普通链接外，也会取小程序按钮（含输入框上方的按钮，自动签名）和登录按钮（自动确认登录）",
      ...
      labelPinnedFirst: "优先使用置顶消息",
      pinnedFirstHint: "勾选后先在该聊天的置顶消息中找按钮（取最新的一条），找不到再按消息范围查找。群里的抽奖、验证等置顶消息在大量新消息后仍可被点击",
      ...
      verifyAiBtnHint: "验证消息里出题（如「9 - 1 = ?」再给一排数字按钮）时，按钮文字事先无法确定，勾选后由 AI 读消息（含图片）再挑按钮。上方可填提示补充说明，如 点击算式答案。拒绝、举报等按钮不会交给 AI，避免误点导致验证失败。若该群改用投票（Quiz）形式出题，会同样交给 AI 读题并投出正确选项，无需另行设置。需在设置中配置 AI API Key",
      ...
      miniAppButtonHint: "在最近 10 条消息中查找该小程序（Mini App）按钮，留空则使用最近的一个。消息下方的内联按钮和输入框上方的回复键盘按钮（如「开始验证」）都会查找；按钮文字包含所填文字即匹配。由 Telegram 签名后用已安装的浏览器打开，从而通过 Cloudflare 验证。回复键盘小程序通过 sendData 把结果交回机器人，本任务会像官方客户端一样代为转发。多种写法用 | 分隔，任意一个匹配即可，如 中验证|in App，适用于机器人按账户语言切换按钮文字的情况",
      labelInAppButton: "小程序内的操作步骤（可选）",
      inAppStepPlaceholder: "例：参与抽奖|Join giveaway、css:#checkin-btn、delay(10000)",
      addInAppStep: "添加步骤",
      appStepsHint: "按列表顺序在小程序页面上执行，每步一张卡片：从下拉框选类型、填该类型的字段、可勾选「失败后继续」。留空则自动识别签到类按钮",
      appThenStepsHint: "条件成立时执行这些步骤",
      appElseStepsHint: "条件不成立时执行这些步骤；留空则什么都不做",
      appStepUnknown: "这一行本版本无法识别，已原样保留：{row}。改变类型会丢弃它",
      appStepLabelText: "按钮文字",
      appStepLabelPlaceholder: "立即签到（多种写法用 | 分隔）",
      appStepLabelHint: "优先匹配真正的按钮/链接，而不是同名的说明文字；多种写法用 | 分隔，任意一个匹配即点击，适用于小程序会按语言切换按钮文字的情况",
      appStepSelector: "CSS 选择器",
      appStepSelectorPlaceholder: "#checkin-btn",
      appStepSelectorHint: "按钮文字不稳定或有重名时更可靠；可放心使用 > 子元素写法",
      appStepDelay: "等待",
      appStepDelayHint: "适用于页面加载较慢的小程序；也可放在 {turnstile} 或条件分支之前，等组件/弹窗渲染出来",
      appStepScrollMode: "滚动方式",
      appStepScrollPixels: "按像素滚动",
      appStepScrollSelector: "滚动到元素",
      appStepScrollHint: "把屏幕外的按钮滚到可见区域。页面内容长度不固定时，滚动到元素比按像素更可靠（元素在内部可滚动面板中也有效）",
      appStepScrollSelectorPlaceholder: "#footer",
      appStepScrollX: "横向 x",
      appStepScrollY: "纵向 y",
      appStepAiHint: "补充说明（可选）",
      appStepAiHintPlaceholder: "签到按钮",
      appStepAiHintHint: "截图后由 AI 判断该点哪个按钮；填了说明会更准",
      appCheckElement: "页面上有该元素",
      appCheckText: "页面文字包含",
      appIfWordsPlaceholder: "已签到（多种写法用 | 分隔）",
      appIfHint: "只看执行到这一步时页面的当前状态，不会等待；小程序弹出较慢时请在本步骤前加「等待」",
      appNegateHint: "勾选后条件取反：页面上「没有」该元素（或没有这些文字）时才执行 then 分支",
      appContinueAfterFailHint: "这一步失败也继续往下走，失败原因仍会记入日志。适用于可能不存在的弹窗、可选的领取按钮等",
      appSummaryOptional: "失败也继续",
      appSummaryArms: "成立 {then} 步 / 不成立 {else} 步",
      appStepType: {
        label: "点击按钮文字",
        css: "点击 CSS 选择器",
        delay: "等待",
        scroll: "滚动页面",
        turnstile: "过 Turnstile 人机验证",
        aiBtn: "由 AI 判断该点哪个按钮",
        input: "自动计算算术验证码",
        aiInput: "由 AI 读题并填入",
        if: "条件分支（if）",
      ...
      appStepHint: {
        ...
        delay: "",
        scroll: "",
        turnstile: "点击小程序页面上的 Cloudflare Turnstile 复选框：通过浏览器协议定位组件（可穿透其跨域 iframe），比让 AI 判断坐标更可靠。IP 良好时 Turnstile 会自动通过、页面上不出现复选框，因此没有验证组件时该步骤视为成功；只会等待很短时间让组件渲染，若小程序弹出较慢，请在本步骤前加「等待」",
        aiBtn: "",
        input: "读取页面上的算术验证码，算出结果并填入",
        aiInput: "把页面上的问题交给 AI 读题并填入答案",
        if: "",
      },
    ...
    web: {
      ...
      labelCaptchaRounds: "验证轮数",
      captchaRoundsPlaceholder: "4",
      labelCaptchaWait: "总时限（毫秒）",
      labelCaptchaHint: "额外提示（可留空）",
      captchaHintPlaceholder: "题目里的 sun 指的是太阳，不是儿子",
      labelCaptchaOptional: "页面上没有验证码时也算通过",
      captchaHint:
        "用于「点击所有含公交车的图片」这类九宫格图片验证码（目前支持 hCaptcha）：先点「我是人类」，随后从验证码自身的 iframe 中读出题目文字、九个图块的位置、参考图与 Verify 按钮，在图块上画好编号，只问模型「哪几个编号符合题目」——模型只需认图，不需要判断坐标，这正是让中等体量的模型也能通过的关键。通常两轮，成功以页面拿到 token 为准；每轮问一次模型，被 AI 网关限流时会自动等待重试",
  ...
  templates: {
    ...
    createJobsNoAccounts: "没有可用账户",
    createJobsNoMatch: "没有符合筛选条件的账户",
    createJobsLinked: "已关联",
    filterExcludeLinked: "排除已关联此模板的账户",
    filterExcludeRestricted: "排除受限账户（受限 / 封禁 / 冻结，轻度限制不算）",
  ...
  logs: {
    ...
    colMessage: "消息",
    noLogs: "暂无日志",
    sizeUnmeasured: "尚未统计（首次升级后会在后台逐步补齐）",
    totalSize: "日志共 {size}",
    compact: "精简此日志（删除截图）",
    compactKeepPrompt: "保留最近多少张截图？0 表示全部删除。",
    compactAll: "精简全部",
    compactAllConfirm: "删除全部 {n} 条日志中的截图？每一步的记录会保留，截图无法恢复。",
    compactSelected: "精简所选 {n} 条",
    compacted: "已删除 {n} 张截图，释放 {size}",
    compactedNone: "没有可删除的截图",
  ...
  settings: {
    ...
    labelKeepScreenshots: "每次运行保留的截图数",
    keepScreenshotsAll: "不限制",
    keepScreenshotsHint:
      "每条日志只保留最近的这么多张截图，0 表示不保留截图。留空则按大小上限自动裁剪。步骤记录不受影响，截图是日志占用空间的主要原因。",
    ...
    tgPoolSection: "Telegram 连接",
    labelTgClientMax: "最大同时连接账号数",
    tgClientMaxHint:
      "最多有多少个账号同时保持 Telegram 连接，默认 8，可设 1-50。超出后会断开最久未使用且无人占用的连接；正在被查看或被任务占用的连接不会被断开。每个连接约占数 MB 内存，调低会更省内存但切换账号时需要重连，而每次重连都会受 Telegram 按出口 IP 的频率限制影响。调低后立即生效。",
    tgPoolConnected: "当前连接数",
    tgPoolNone: "当前没有账号保持连接。",
    tgPoolAccount: "账号",
    tgPoolState: "状态",
    tgPoolHeldBy: "占用者",
    tgPoolIdle: "空闲时长",
    tgPoolOffline: "已断开",
    tgPoolViewers: "{n} 个查看者",
    tgPoolLeases: "{n} 个任务",
    tgPoolIdleHolder: "无人占用",
    tgPoolHint:
      "无人占用的连接在空闲 {n} 分钟后自动断开。若某个连接长时间被任务占用（时长显示为数小时），说明有任务卡住后没有释放，超过 3 小时会被自动收回并在日志中记录。",
    tgPoolUnavailable: "无法读取连接状态。",
    memorySection: "内存使用",
    ...
    xray: {
      ...
      reinstall: "重新安装",
      installing: "下载中…",
      installed: "内核已安装，已重新拉取订阅。",
      fromData: "来自数据目录",
      fromHost: "来自系统",
      unsupported: "当前平台（{platform}）没有官方构建，请自行安装并加入 PATH",
      waiting: "有 {count} 个节点已保存但未启动：它们需要此内核。安装后即可启用。",
      hint: "Bemby 自带 VLESS over WebSocket 客户端（即 Cloudflare Workers 部署所提供的那种）。机场订阅里常见的其他节点 —— 尤其是 VLESS + REALITY，以及 VMess、Trojan、Shadowsocks —— 需要真正的内核，因此按需将 Xray 下载到数据目录（下载约 20MB，占用约 40MB），升级镜像后无需重装。它以独立进程运行，同样为每个节点开一个本机 SOCKS5 端口。",
    },
    providersHint: "配置服务商后可一键导入其当前代理列表。Webshare 使用官方 API（填 API Token）；「列表地址」适用于任何返回文本列表的下载链接（支持 ip:port、ip:port:用户:密码、用户:密码@ip:port，可带 scheme://），因此其他服务商也能使用；「节点订阅」读取节点订阅：既支持部署在 Cloudflare Workers 上的 edgetunnel，也支持机场提供的 VLESS（含 REALITY）、VMess、Trojan、Shadowsocks 节点；每个节点会在本机开一个 SOCKS5 端口，之后与普通代理一样使用（Telegram 亦可）。除 VLESS over WebSocket 外，其余节点需要上方的节点内核。地址中写 `{token}` 可从下方 Token 框取值填入（路径或查询参数均可，会自动转义），这样令牌不会明文显示在地址里；地址中没有 `{token}` 时，Token 会以 `Authorization: Bearer` 请求头发送。导入项按服务商覆盖，手动添加的代理不受影响。过 Cloudflare 时不同出口 IP 成功率差异较大，代理池越大越容易找到可用的；但订阅节点的出口是 Cloudflare 自己的 IP 段，用于过验证通常效果不佳，更适合连接 Telegram 或绕开本机网络限制。",
    providerName: "名称",
...
const en: typeof zh = {
  ...
  accounts: {
    ...
    filters: {
      btn: "Filters",
      clear: "Clear filters",
      anyStatus: "Any auth status",
      needsAuth: "Needs auth (not authenticated)",
      anyRestriction: "Any restriction",
      restrictedAny: "Restricted (any)",
      unchecked: "Never checked",
      anyEmail: "Any email state",
      emailBemby: "Has Bemby email",
      emailHas: "Has login email",
      emailNone: "No login email",
      anyPasskey: "Any passkey state",
      passkeyBemby: "Has Bemby passkey",
      passkeyHas: "Has passkey",
      passkeyNone: "No passkey",
      enabledLabel: "Enabled",
      anyEnabled: "Enabled and disabled",
      enabledOnly: "Enabled only",
      disabledOnly: "Disabled only",
      proxyLabel: "Proxy",
      anyProxy: "Any proxy state",
      proxyAssigned: "Proxy assigned",
      proxyNone: "No proxy",
      ownershipLabel: "Source and ownership",
      anyOwnership: "Any source",
      ownershipImported: "Session-imported",
      ownershipPending: "Ownership pending",
      ownershipTaken: "Ownership taken",
      ownershipManual: "Added manually",
    },
    ...
    importCards: {
      btn: "Import Cards",
      title: "Import Cards (Session Accounts)",
      hint: "One card per line, formatted as phone----getcode-URL (a bare link or UUID also works). Each card is redeemed at the converter for a real session; the account is verified by connecting, then saved already authenticated -- no verification code is ever needed. The device fingerprint and API ID are pinned from the card, so do not sign these accounts in on the official app.",
      start: "Start Import",
      startFailed: "Failed to start import",
      notesLabel: "Notes template",
      notesHint: "{phone} and {uuid} are replaced per account",
      aliveCheckLabel: "Check liveness before converting",
      aliveCheckHint: "Screens each card first so dead/frozen ones are not spent -- downloading a card's files forfeits its free replacement.",
      proxyHint: "Proxy used to verify each login, assigned at random; empty uses the global exit. SOCKS only.",
      converterUrlLabel: "Converter service URL",
      converterUrlPlaceholder: "https://your-converter.example",
      converterUrlHint: "Address of the card-to-session converter, which you supply. The last value is remembered; it is never stored in the codebase.",
      status: {
        pending: "Pending",
        checking: "Checking",
        frozen: "Frozen",
    ...
    takeOwnership: {
      btn: "Take Ownership",
      title: "Take Ownership (Card Accounts)",
      intro: "Locks {n} card-imported account(s) to you: change the 2FA from the seller's original (stored at import) to your own, terminate every other session, and remove non-Bemby passkeys. After this the seller can no longer get in.",
      skippedHint: "{n} selected account(s) skipped -- only accounts imported as session cards can be taken over.",
      randomiseLabel: "Generate a random password per account",
      randomiseHint: "Each account gets its own strong random 2FA (kept in Bemby). Safer than one shared password.",
      newPassword: "New 2FA password",
      repeatPassword: "Repeat new password",
      removePasskeys: "Remove non-Bemby passkeys",
      addPasskey: "Register a Bemby passkey after taking over",
      notesPlaceholder: "ownership taken",
      warning: "This really changes the 2FA on Telegram and kicks off other sessions -- it cannot be undone. Best run 3-5 days after import. Do not sign these accounts in on the official app beforehand.",
      start: "Start Take Ownership",
      noTargets: "None of the selected accounts are card-imported and authenticated.",
    },
    bulkEmail: {
  ...
  jobs: {
    ...
    runEveryDaysOneTimeHint: "One time job: runs once on a random day within this many days from today (0 is today, e.g. 0-5), regardless of when it last succeeded; switches off after that run",
    ...
    custom: {
      ...
      actionSaveMessageUrl: "Save link from message as variable",
      ...
      saveUrlVarHint:
        "The address is stored as {v}; write {v} in a later Open URL or page step to use it. Besides plain links this also takes Mini App buttons (including the keyboard above the composer, signed) and login buttons (the login is confirmed for you)",
      ...
      labelPinnedFirst: "Use pinned messages first",
      pinnedFirstHint: "Looks for the button among the chat's pinned messages first (newest wins) and only then within the message scope. Keeps a pinned draw or prompt reachable however many messages follow it",
      ...
      verifyAiBtnHint: "When the prompt asks a question -- \"9 - 1 = ?\" over a row of numbers -- no button text can be written down in advance, so the AI reads the message (pictures included) and names the button to press. The field above becomes an optional hint, e.g. press the answer to the sum. Buttons that decline or report are never offered to the AI, so a wrong pick cannot fail the verification outright. A group that poses the same question as a quiz poll is handled the same way -- the AI reads it and the answer is voted rather than clicked -- with nothing to configure. Needs an AI API key in Settings",
      ...
      miniAppButtonHint: "Looks for that Mini App button in the last 10 messages, or takes the most recent one when blank. Both the inline buttons under a message and the reply keyboard above the composer (a \"开始验证\" button, say) are searched, and a button matches when its text contains what is given. Telegram signs the URL and it is opened in the installed browser, which is what passes the Cloudflare check. A reply-keyboard app hands its result back to the bot with sendData, which the job relays as the official client would. Several wordings can be given separated by | -- 中验证|in App takes whichever the bot rendered, for a bot that follows the account's language",
      ...
      appStepsHint: "Run on the Mini App page in the order listed, one card per step: pick the kind from the dropdown, fill in its own fields, and tick \"carry on when this fails\" where it does not matter. An empty list auto-detects a checkin-worded control",
      appThenStepsHint: "Run these steps when the condition holds",
      appElseStepsHint: "Run these when it does not; leave empty to do nothing",
      appStepUnknown: "This build cannot read this row, so it is kept exactly as it came: {row}. Changing the kind discards it",
      appStepLabelText: "Button text",
      appStepLabelPlaceholder: "Check in (separate wordings with |)",
      appStepLabelHint: "Presses the control carrying that label, preferring a real button or link over a caption that happens to read the same. Several wordings separated by | press whichever the app has, which is how one step covers an app that changes language",
      appStepSelector: "CSS selector",
      appStepSelectorPlaceholder: "#checkin-btn",
      appStepSelectorHint: "More reliable where the label moves about or two controls read the same; a child combinator such as .card > button is fine here",
      appStepDelay: "Wait",
      appStepDelayHint: "For an app that is slow to settle. Also worth putting before a Turnstile or a branch, to give the widget or popup time to render",
      appStepScrollMode: "Scroll by",
      appStepScrollPixels: "A distance in pixels",
      appStepScrollSelector: "Until an element is in view",
      appStepScrollHint: "Brings a control that sits off screen into reach. Where the page's length depends on its content, scrolling to an element beats counting pixels -- and it works for a target inside a scrollable panel",
      appStepScrollSelectorPlaceholder: "#footer",
      appStepScrollX: "Horizontal (x)",
      appStepScrollY: "Vertical (y)",
      appStepAiHint: "What to look for (optional)",
      appStepAiHintPlaceholder: "the checkin button",
      appStepAiHintHint: "Screenshots the page and lets the AI pick the button to press; saying which one makes it surer",
      appCheckElement: "The page has this element",
      appCheckText: "The page says these words",
      appIfWordsPlaceholder: "Checked in (separate wordings with |)",
      appIfHint: "Judged against the page as it stands at this step, with no waiting -- put a Wait before it where the app is slower than that",
      appNegateHint: "Asks the other way round: run the then steps when the page does NOT have it",
      appContinueAfterFailHint: "Carry on with the next step when this one fails; what went wrong is still logged. For a popup that may not be there, an optional claim button, and the like",
      appSummaryOptional: "carries on if it fails",
      appSummaryArms: "{then} then / {else} else",
      appStepType: {
        label: "Press a button by its text",
        css: "Press a CSS selector",
        delay: "Wait",
        scroll: "Scroll the page",
        turnstile: "Pass a Turnstile checkbox",
        aiBtn: "Let the AI pick the button",
        input: "Solve an arithmetic captcha",
        aiInput: "Let the AI answer the question",
        if: "Branch (if)",
      },
      appStepHint: {
        label: "",
        ...
        delay: "",
        scroll: "",
        turnstile: "Ticks a Cloudflare Turnstile checkbox on the app page, located through the browser's own protocol so it reaches inside the cross-origin frame Turnstile draws in, rather than by guessing at coordinates. The step passes when there is no checkbox to tick, since Turnstile clears itself for an address it likes; it waits only a moment for one to render, so put a Wait before it where the app is slower than that",
        aiBtn: "",
        input: "Reads the arithmetic captcha off the page, works it out and types the answer",
        aiInput: "Hands the question on the page to the AI and types what it answers",
        if: "",
      },
    ...
    web: {
      ...
      web_ms_oauth2: "Confirm the mailbox is connected (msOauth2api)",
        ...
        ai_web_captcha: "AI solves the picture captcha (tile grid)",
      ...
      labelCaptchaRounds: "Challenge rounds",
      captchaRoundsPlaceholder: "4",
      labelCaptchaWait: "Overall budget (ms)",
      labelCaptchaHint: "Extra context for the model (optional)",
      captchaHintPlaceholder: "\"sun\" in the question means the star, not a son",
      labelCaptchaOptional: "Pass when there is no captcha on the page",
      captchaHint:
        "For the nine-tile picture captchas -- \"click every tile that shows a bus\" (hCaptcha today). It presses \"I am human\", then reads the challenge out of the captcha's own iframe: the question in words, where the tiles are, the reference picture and the Verify button. A number is drawn on each tile and the model is asked only which numbers answer the question, so it never has to judge a coordinate -- which is what makes this work with a model of ordinary weight. Two rounds is the usual ask, and success is the page holding a token. Each round costs one call to the model, and a rate-limited gateway is waited out rather than failed",
  ...
  templates: {
    ...
    createJobsNoAccounts: "No accounts available",
    createJobsNoMatch: "No accounts match the filters",
    createJobsLinked: "Linked",
    filterExcludeLinked: "Exclude accounts already linked to this template",
    filterExcludeRestricted: "Exclude restricted accounts (limited / blocked / frozen, low limited stays)",
  ...
  logs: {
    ...
    colMessage: "Message",
    noLogs: "No logs yet",
    sizeUnmeasured: "not measured yet (filled in the background after an upgrade)",
    totalSize: "{size} of logs",
    compact: "Compact this log (drop its screenshots)",
    compactKeepPrompt: "How many of the most recent screenshots to keep? 0 drops them all.",
    compactAll: "Compact all",
    compactAllConfirm: "Drop the screenshots from all {n} logs? What each step did is kept; the pictures cannot be brought back.",
    compactSelected: "Compact {n}",
    compacted: "Dropped {n} screenshot(s), freeing {size}",
    compactedNone: "No screenshots left to drop",
  ...
  settings: {
    ...
    labelKeepScreenshots: "Screenshots to keep per run",
    keepScreenshotsAll: "no limit",
    keepScreenshotsHint:
      "Each run's log keeps only its most recent screenshots; 0 keeps none. Leave blank to let the size budget trim them. What each step did is kept either way, and the pictures are what make a log big.",
    ...
    tgPoolSection: "Telegram Connections",
    labelTgClientMax: "Maximum connected accounts",
    tgClientMaxHint:
      "How many accounts keep a Telegram connection at once. Default 8, up to 50. Past that the least recently used connection nobody is holding is dropped; one being viewed or used by a job is never dropped. Each connection is a few MB, so lowering this saves memory but means reconnecting when you switch accounts, and every reconnect counts against Telegram's per-exit-IP rate limit. Applies straight away.",
    tgPoolConnected: "Connected now",
    tgPoolNone: "No account is holding a connection.",
    tgPoolAccount: "Account",
    tgPoolState: "State",
    tgPoolHeldBy: "Held by",
    tgPoolIdle: "Idle",
    tgPoolOffline: "Disconnected",
    tgPoolViewers: "{n} viewer(s)",
    tgPoolLeases: "{n} job(s)",
    tgPoolIdleHolder: "nobody",
    tgPoolHint:
      "A connection nobody is holding is dropped after {n} minutes idle. A connection held by a job for hours means a run hung without releasing it; past 3 hours it is reclaimed automatically and noted in the log.",
    tgPoolUnavailable: "Could not read the connection state.",
    ...
    xray: {
      label: "Node core (Xray)",
      present: "Installed",
      missing: "Not installed",
      install: "Install",
      reinstall: "Install again",
      installing: "Downloading...",
      installed: "The core is installed. The subscriptions were pulled in again.",
      fromData: "from the data dir",
      fromHost: "from the host",
      unsupported: "No published build for this platform ({platform}); install one by hand and put it on PATH",
      waiting: "{count} node(s) are stored but not running: they need this core. Install it, and they come up.",
      hint: "Bemby carries VLESS-over-WebSocket nodes itself, which is what a Cloudflare Workers deployment serves. Everything a commercial seller serves -- VLESS behind REALITY above all, plus VMess, Trojan and Shadowsocks -- needs a real core, so Xray is fetched into the data dir on demand (about 20MB to download, 40MB on disk) and an upgrade does not have to fetch it again. It runs as a separate process and gives each node a local SOCKS5 port, exactly as the built-in bridge does.",
    },
    providersHint: "A configured provider's current list can be pulled in on demand. Webshare uses its official API (enter an API token); \"List URL\" works with any download link that returns a text list (ip:port, ip:port:user:pass, user:pass@ip:port, with or without a scheme://), so other providers work too; \"Node subscription\" reads a node subscription -- edgetunnel on Cloudflare Workers, or a commercial seller's VLESS (REALITY included), VMess, Trojan and Shadowsocks nodes -- and gives each node a local SOCKS5 port that is used like any other proxy, Telegram included. Everything but VLESS-over-WebSocket needs the node core above. Write `{token}` anywhere in a URL, in the path or in a query parameter, and the token field below fills it in (escaped), so the secret is not on show in the address; a URL without `{token}` gets the token as an `Authorization: Bearer` header instead. Each import replaces only that provider's entries; manually added proxies are left alone. Cloudflare accepts some exit IPs and not others, so a larger pool makes a working one easier to find -- though a subscription's nodes all leave from Cloudflare's own addresses, which makes them a poor choice for clearing its checks and a good one for reaching Telegram or getting past a blocked host network.",
    providerName: "Name",
  ...
  tgc: {
    ...
    poll: {
      ...
      multiple: "Pick any number",
      noVotes: "Nobody answered yet",
      totalVoters: "{n} voted",
      submit: "Vote",
      failed: "Vote failed",
    },
