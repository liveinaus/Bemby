...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      verifyMentionsMeHint: "同一时间有多人入群时，群里会为每个人各发一条验证消息，此时可能点到别人的按钮。勾选后只点击 @ 到本账户的那条（@用户名、未设用户名时的文字提及，或数字 ID），其它人的消息会跳过并继续等待（受下面的等待时长限制）。需先填写上方的验证按钮，否则不进行验证",
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      verifyMentionsMeHint: "When several people join at once the group posts one verification message each, and the wrong one can get clicked. Enabled, only the prompt naming this account is clicked (@username, a text mention when no username is set, or the numeric ID); prompts for other people are skipped and the wait continues, bounded by the wait below. Needs the verification button above to be filled in, otherwise no verification runs at all",
