...
const zh = {
  ...
  settings: {
    ...
    providersHint: "配置服务商后可一键导入其当前代理列表。Webshare 使用官方 API（填 API Token）；「列表地址」适用于任何返回文本列表的下载链接（支持 ip:port、ip:port:用户:密码、用户:密码@ip:port，可带 scheme://），因此其他服务商也能使用。导入项按服务商覆盖，手动添加的代理不受影响。过 Cloudflare 时不同出口 IP 成功率差异较大，代理池越大越容易找到可用的。",
    providerName: "名称",
