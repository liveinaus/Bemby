...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      addInAppStep: "添加步骤",
      inAppButtonHint: "打开小程序页面后按列表顺序执行，每行一个步骤。可用步骤：①按钮文字，如 立即签到（优先匹配真正的按钮/链接，而不是同名的说明文字）；多种写法用 | 分隔，任意一个匹配即点击，如 参与抽奖|加入抽奖|Join giveaway，适用于小程序会按语言切换按钮文字的情况；②css: 选择器，如 css:#checkin-btn（每行独立成步，选择器里可放心使用 > 子元素写法）；③delay(10000) 等待 10 秒再执行下一步（也可写 delay(10s)），适用于页面加载较慢的小程序；④scroll(css:#reply) 滚动到指定元素（页面内容长度不固定时比按像素更可靠，元素在内部可滚动面板中也有效）；scroll(0, 800) 按像素滚动页面（第一个数字为横向 x，第二个为纵向 y；只写一个数字表示纵向，如 scroll(800)；也可写 scroll(y=800)、scroll(x=-200)；负数为反向滚动），用于把屏幕外的按钮滚到可见区域，数值超过页面长度时直接滚到尽头，例如 scroll(0, 99999) 滚到底部；⑤{turnstile} 点击小程序页面上的 Cloudflare Turnstile 人机验证复选框：通过浏览器协议定位组件（可穿透其跨域 iframe），比让 AI 判断坐标更可靠，适用于小程序在流程中途才弹出验证的情况；IP 良好时 Turnstile 会自动通过、页面上不出现复选框，因此没有验证组件时该步骤视为成功；只会等待很短时间让组件渲染，若小程序弹出较慢，请在本步骤前加 delay()；⑥{aiBtn} 截图后由 AI 判断该点哪个按钮，可用 {aiBtn:签到按钮} 补充说明；⑦{input} 自动计算算术验证码并填入；⑧{aiInput} 由 AI 读题并填入。常用组合：delay(10000) 后接 {aiBtn}（等页面加载完再让 AI 找按钮）；scroll(0, 99999) 后接 立即签到（先滚到底部再点按钮）。列表留空则自动识别签到类按钮。按钮已置灰或显示已完成时不会点击；若只点到普通文字且页面没有反应，该操作会报错而不是误报成功",
