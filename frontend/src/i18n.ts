...
const zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      verifyButtonHint: "部分群组入群后会发送验证消息，需点击按钮完成验证。填写按钮文字（部分匹配即可），加入后将等待并点击该按钮；若只有一个按钮则直接点击。留空则不处理。若该按钮是「在私信中验证」这类跳转链接，会按真实客户端的做法给机器人发送 /start，并在私聊里继续完成验证（需要时会加入其指定的频道，再点击完成验证）",
      ...
      miniAppButtonHint: "在最近 10 条消息中查找该小程序（Mini App）按钮，留空则使用最近的一个。由 Telegram 签名后用已安装的浏览器打开，从而通过 Cloudflare 验证",
...
const en: typeof zh = {
  ...
  jobs: {
    ...
    custom: {
      ...
      verifyButtonHint: "Some groups post a verification message after you join that requires clicking a button. Enter the button text (partial match is fine); after joining, the job waits for and clicks that button, or clicks the sole button if there is only one. Leave blank to skip. When that button is a link such as \"verify in a private chat\", it is followed the way a real client does -- /start is sent to the bot and the verification continues in the private chat, joining the channel it asks for and pressing its verify button",
      ...
      miniAppButtonHint: "Looks for that Mini App button in the last 10 messages, or takes the most recent one when blank. Telegram signs the URL and it is opened in the installed browser, which is what passes the Cloudflare check",
